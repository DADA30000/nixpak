...
in
{
  ...
  config = {
    system-dbus.args =
      (mapAttrsToList (n: v: "--${v}=${n}") config.system-dbus.policies)
      ++

        (flatten (mapAttrsToList (n: v: map (x: "--call=${n}=${x}") v) config.system-dbus.rules.call))
      ++

        (flatten (
          mapAttrsToList (n: v: map (x: "--broadcast=${n}=${x}") v) config.system-dbus.rules.broadcast
        ));
    dbus.args =
      (mapAttrsToList (n: v: "--${v}=${n}") config.dbus.policies)
      ++

        (flatten (mapAttrsToList (n: v: map (x: "--call=${n}=${x}") v) config.dbus.rules.call))
      ++

        (flatten (mapAttrsToList (n: v: map (x: "--broadcast=${n}=${x}") v) config.dbus.rules.broadcast));
  };
}
