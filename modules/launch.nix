...
let
  ...
  bwrapArgsJson =
    if config.bubblewrap.bindEntireStore then
      ...
      pkgs.runCommand "bwrap-args.json"
        {
      ...
      (''
        makeWrapper ${launcher}/bin/launcher $out${executablePath} \
          ${concatStringsSep " " (flatten [
            ...
            (optionals config.system-dbus.enable "--set XDG_SYSTEM_DBUS_PROXY_EXE ${systemDbusProxyWrapper}")
  ...
  '';
  systemDbusProxyWrapper = pkgs.writeShellScript "xdg-system-dbus-proxy-wrapper" ''
    exec ${config.bubblewrap.package}/bin/bwrap ${
      concatStringsSep " " (flatten [
        (bindRo "/etc")
        (bindRo "/nix/store")
